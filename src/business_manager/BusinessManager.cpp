...
ListenId BusinessManager::ListenAircraftAttitude(const AircraftAttitudeMsgCallback &on_messages_callback, const OnSubscribeResultCallback &on_result_callback, const std::string &device_sn, NotifactionFrequency freq) {
    auto strong_topic_mgr = topic_mgr_.lock();
    if (strong_topic_mgr == nullptr) {
        NC_LOG_INFO("[BusinessManager] ListenAircraftAttitude failed, no topic_mgr_");
        return NotificationCenterErrorCode_NotConnected;
    }

    // 使用新的Observe接口，自动处理订阅逻辑
    const SubscribeTopicTuple tuple{device_sn, "aircraft_attitude"};
    ...
    auto listen_id = strong_topic_mgr->Observe(tuple, freq, [this, weak_ptr, on_msg_cb = on_messages_callback](const std::shared_ptr<PublishTopicWrapper>& message)->void {
        ...
        if (on_msg_cb && message) {
            const auto msg = std::make_shared<PublishAircraftAttitudeTopic>(message);
            NC_LOG_INFO("[BusinessManager] ListenAircraftAttitude observe: %s", msg->ToJsonString().c_str());
            on_msg_cb(msg->msg);
        }
...
ListenId BusinessManager::ListenAircraftSpeed(const AircraftSpeedMsgCallback &on_messages_callback, const OnSubscribeResultCallback &on_result_callback, const std::string &device_sn, NotifactionFrequency freq) {
    auto strong_topic_mgr = topic_mgr_.lock();
    if (strong_topic_mgr == nullptr) {
        NC_LOG_INFO("[BusinessManager] ListenAircraftSpeed failed, no topic_mgr_");
        return NotificationCenterErrorCode_NotConnected;
    }

    // 使用新的Observe接口，自动处理订阅逻辑
    const SubscribeTopicTuple tuple{device_sn, "aircraft_speed"};

    WeakDummy(weak_ptr);
    auto listen_id = strong_topic_mgr->Observe(tuple, freq, [this, weak_ptr, on_msg_cb = on_messages_callback](const std::shared_ptr<PublishTopicWrapper>& message)->void {
        WeakDummyReturn(weak_ptr);
        if (on_msg_cb && message) {
            const auto msg = std::make_shared<PublishAircraftSpeedTopic>(message);
            NC_LOG_INFO("[BusinessManager] ListenAircraftSpeed observe: %s", msg->ToJsonString().c_str());
            on_msg_cb(msg->msg);
        }
    }, [this, weak_ptr, on_result_cb = on_result_callback](int err)->void {
        WeakDummyReturn(weak_ptr);
